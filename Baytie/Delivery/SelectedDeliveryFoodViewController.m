//
//  SelectedDeliveryFoodViewController.m
//  Baytie
//
...
@interface SelectedDeliveryFoodViewController (){
    NSMutableArray *arrDeliveryFoods;
}

@end

@implementation SelectedDeliveryFoodViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view from its nib.
    [self.navigationItem setHidesBackButton:YES];
    arrDeliveryFoods = [[NSMutableArray alloc] init];
...
- (void)getAllFood{
    [arrDeliveryFoods removeAllObjects];
        ...
        if ([[_responseObject objectForKey:@"code"] integerValue] == 0) {
            for (NSDictionary *dict in [_responseObject objectForKey:@"resource"]) {
                //if ([dict objectForKey:@"price"] && [[dict objectForKey:@"price"] floatValue] > 0) {
                    [arrDeliveryFoods addObject:dict];
                //}
            }
            [KindsSelectedFoodTableView reloadData];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSDictionary *dict = [arrDeliveryFoods objectAtIndex:indexPath.row];
    if (cell == nil) {
...
//    SelectedFoodPreViewViewController *viewcontroller = [[SelectedFoodPreViewViewController alloc] initWithNibName:@"SelectedFoodPreViewViewController" bundle:nil];
//    viewcontroller.deliveryFoodsItem = [arrDeliveryFoods objectAtIndex:indexPath.row];
...
//    [self.navigationController pushViewController:viewcontroller animated:YES];
    ...
    SelectedFoodPreViewFitViewController *viewcontroller = [[SelectedFoodPreViewFitViewController alloc] initWithNibName:@"SelectedFoodPreViewFitViewController" bundle:nil];
    viewcontroller.deliveryFoodsItem = [arrDeliveryFoods objectAtIndex:indexPath.row];
    viewcontroller.selectedRestroName = _selectedRestroName;
    viewcontroller.locationId = [_parMenuFoodItem objectForKey:@"location_id"];
    viewcontroller.areaId = _areaId;
