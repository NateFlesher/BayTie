...
@interface LoyaltyPointsViewController (){
    NSMutableArray *arrLoyaltyPoints;
}
@end
...
- (void)viewDidLoad {
    ...
    arrLoyaltyPoints = [[NSMutableArray alloc] init];
}
...
- (void)getLoyaltyPoints{
    [arrLoyaltyPoints removeAllObjects];
    ...
    void ( ^successed )( id _responseObject ) = ^( id _responseObject )
    {
        if ([[_responseObject objectForKey:@"code"] integerValue] == 0) {
            ...
            arrLoyaltyPoints = [_responseObject objectForKey:@"resource"];
            [loyaltyPointsTableView reloadData];
    ...
    [[Communication sharedManager] GetOrdersMyPoints:APPDELEGATE.access_token serviceType:0 pointType:2 successed:successed failure:failure];
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSDictionary *dict = [arrLoyaltyPoints objectAtIndex:indexPath.row];
    if (cell == nil) {
    ...
    cell.lblGainedPoint.text =[APPDELEGATE getIntToStringForPoint:[[dict objectForKey:@"gained_loyalty_point"] integerValue]];
    cell.lblBalance.text =[APPDELEGATE getIntToStringForPoint:[[dict objectForKey:@"balance_loyalty_point"] integerValue]];
    return cell;
}
