//
//  SelectedFoodForPickUpPreViewController.m
//  Baytie
//
...
@interface SelectedFoodForPickUpPreViewController ()<UITextFieldDelegate,UIGestureRecognizerDelegate, UITableViewDelegate, UITableViewDataSource>{
...
@implementation SelectedFoodForPickUpPreViewController
...
- (void)getFoodInfo{
        ...
        if ([[_responseObject objectForKey:@"code"] integerValue] == 0) {
            ...
            if ([[[_responseObject objectForKey:@"resource"] objectForKey:@"price_type"] integerValue] == 2) {
             ...
             if ([[_responseObject objectForKey:@"resource"] objectForKey:@"variations"] && [[[_responseObject objectForKey:@"resource"] objectForKey:@"variations"] count]>0) {
                 ...
                 [foodItemTableview setFrame:CGRectMake(8, 31, 303, 40 * [arrCategories count])];
    ...
    [[Communication sharedManager] GetRestaurantsFoodItem:APPDELEGATE.access_token foodItemId:[[_pickupFoodsItem objectForKey:@"id"] integerValue] successed:successed failure:failure];
}
...
//    }
    ...
    [[Communication sharedManager] SetOrdersCart:APPDELEGATE.access_token serviceType:APPDELEGATE.serviceType productId:[[_pickupFoodsItem objectForKey:@"id"] integerValue] quantity:[itemCount.text integerValue] variationIds:variationIDS spacialRequest:txtSpecialRequest.text successed:successed failure:failure];
}
...
{
    if ([tableView isEqual:foodItemTableview]) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([tableView isEqual:foodItemTableview]) {static NSString *simpleTableIdentifier = @"CatergoryItem";
...
- (IBAction)onMinusCount:(id)sender {
    ...
    itemCount.text = [NSString stringWithFormat:@"%d", amount];
    CGFloat sum = 0;
    for (NSDictionary *dict in selectedVars) {
        for (NSDictionary *dc in [dict objectForKey:@"selected"]) {
            sum = sum + [[dc objectForKey:@"price"] floatValue];
